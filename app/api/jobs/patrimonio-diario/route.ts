...
async function manejar(request: Request) {
  ...
  try { const admin = createAdminClient(); const usuarios = await getUsuariosJob(admin); if (!usuarios.length) return NextResponse.json({ ok: false, reason: "No hay usuarios en la app." }, { status: 500 }); const resultados = await ejecutarPorUsuario(admin, usuarios, () => ejecutarPatrimonioDiario(admin, { targetDate })); return NextResponse.json({ ok: resultados.every(r => r.ok), usuarios: resultados.length, resultados }); }
