...
function autorizado(request: Request): boolean {
  ...
  return new URL(request.url).searchParams.get("token") === esperado;
}
...
function detalleError(e: unknown): string {
  ...
  if (e && typeof e === "object") {
    ...
    const p = [o.message, o.details, o.hint, o.code].filter(Boolean);
...
async function manejar(request: Request) {
  ...
  if (targetDate && !/^\d{4}-\d{2}-\d{2}$/.test(targetDate)) {
    return NextResponse.json(
      { error: "Parámetro date inválido (YYYY-MM-DD)." },
      { status: 400 }
    );
  }
