...
import { Check, Crown, Sparkle, Lightning } from "@phosphor-icons/react/dist/ssr";
        ...
        {PLANES.map((plan) => {
          ...
          return (
            <article key={plan.nombre} className={`relative flex flex-col rounded-2xl border p-6 shadow-sm transition-transform hover:-translate-y-1 ${plan.destacado ? "border-primary bg-primary/[0.04] ring-2 ring-primary/20" : "border-border bg-card"}`}>
              ...
              <button type="button" className={`mt-6 w-full rounded-xl px-4 py-2.5 text-sm font-bold transition-opacity hover:opacity-90 ${plan.destacado ? "bg-primary text-primary-foreground" : "border border-border bg-background hover:bg-accent"}`}>Elegir plan</button>
            </article>
          );
        })}
      </div>
