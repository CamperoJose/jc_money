...
import { Crown, Sparkle, Gift, ArrowRight, Flower, Heart, Star } from "@phosphor-icons/react";
import { Dialog, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from "@/components/ui/dialog";

export function ProWelcome() {
  const [open, setOpen] = useState(true);
  const [nombre, setNombre] = useState("Princesa");
      ...
      <div className="overflow-hidden rounded-2xl">
        <div className="relative -mx-6 -mt-6 mb-6 min-h-[255px] overflow-hidden bg-gradient-to-br from-fuchsia-600 via-pink-500 to-violet-700 px-6 pb-8 pt-7 text-white">
          ...
          <div className="absolute inset-0 overflow-hidden" aria-hidden="true">
            {flores.map((flor, index) => (
              <span
                key={`${flor}-${index}`}
                className="absolute animate-bounce text-2xl drop-shadow-sm"
                style={{ left: `${5 + index * 8}%`, top: `${18 + (index % 4) * 17}%`, animationDelay: `${index * 120}ms`, animationDuration: `${2 + (index % 3) * 0.6}s` }}
              >
                {flor}
              </span>
            ))}
          ...
          <div className="relative flex flex-col items-center text-center">
            <div className="mb-4 flex size-20 animate-pulse items-center justify-center rounded-full bg-white/20 shadow-2xl ring-4 ring-white/20 backdrop-blur-sm">
              <Gift className="size-11" weight="fill" />
            </div>
        ...
        <DialogHeader className="mb-5 text-center sm:text-center">
          ...
          <DialogDescription className="mt-2 leading-6">
            Por ser toda una princesa, tienes un <strong>regalo especial de bienvenida</strong>: una licencia de MyMoney PRO de por vida para que disfrutes todo sin límites.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-3">
          <div className="relative overflow-hidden rounded-2xl border border-pink-200 bg-gradient-to-r from-pink-50 via-fuchsia-50 to-violet-50 p-5 dark:border-pink-900/50 dark:from-pink-950/30 dark:via-fuchsia-950/20 dark:to-violet-950/30">
            ...
            <div className="flex items-start gap-3">
              <div className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-pink-500/10 text-pink-500"><Flower className="size-6" weight="fill" /></div>
              <div>
                <p className="font-bold">Tu regalo de bienvenida 🎁</p>
                <p className="mt-1 text-sm text-muted-foreground">MyMoney PRO · acceso de por vida · sin fecha de vencimiento</p>
              </div>
            </div>
          </div>
        </div>
